...
uint_pitch  pitch_sign[NUM_PITCH_UINTS];
...
{
    memset((void *)pitch_sign,'\000',sizeof(pitch_sign));
...
void edge_autocorrelation(void)
{
    ...
    pitch_autocor_size = 0;
    uint cur_entry = 0;
    for (uint i=0;i<pce;i++)
    {
        int autocor = 0;
        uint bytshift = (i - j) / NUM_PITCH_BITS_PER_UINT;
        uint bitshift = (i - j) & (NUM_PITCH_BITS_PER_UINT - 1);
        for (uint j=i;j<pce;j++)
        {
            autocor += count_bits(pitch_sign[
        }
    }
